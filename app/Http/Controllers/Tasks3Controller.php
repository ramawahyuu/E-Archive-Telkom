...
class Tasks3Controller extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($month) {
            $tasks = Task3::whereMonth('tgl_mulai_r', $month)
                ->orWhereMonth('tgl_mulai_p', $month)
                ->get();
        } else {
            $month = $request->input('month', date('n')); // Default to current month if no month is selected
            $tasks = Task3::whereMonth('tgl_mulai_r', $month)->get();
        }

        return view('tasks3', compact('tasks', 'daysInMonth'));
    }
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('tasks3.index');
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        return redirect()->route('tasks3.index')->with('success', 'Informasi berhasil diperbarui.');
    }
    // Delete task
    public function destroy(Task3 $task)
    {
        $task->delete();
        return redirect()->route('tasks3.index');
    }
